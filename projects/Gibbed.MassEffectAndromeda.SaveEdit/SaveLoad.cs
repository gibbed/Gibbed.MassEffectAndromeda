    ...
    internal class SaveLoad : PropertyChangedBase
    {
        ...
        public SaveLoad()
        {
            ...
            if (string.IsNullOrEmpty(savePath) == false)
            {
                ...
                savePath = Path.Combine(savePath, "Mass Effect Andromeda", "Save");
        ...
        public IEnumerable<IResult> OpenFile(Action<string> fileNameAction, Action<int> filterIndexAction)
        {
            fileNameAction(null);
            ...
            var ofr = new MyOpenFileResult()
                .FilterFiles(
                    ffc => ffc.AddFilter("Career*", this._FilterIndex == 1)
                              .WithDescription("Save Files")
                              .AddFilter("*.json", this._FilterIndex == 2)
                              .WithDescription("Raw Save Files"))
            ...
            var chosenPath = Path.GetDirectoryName(fileName);
            this._SavePath = chosenPath;
            this._FilterIndex = filterIndex;

            fileNameAction(fileName);
            filterIndexAction(filterIndex);
        }

        public IEnumerable<IResult> SaveFile(Action<string> fileNameAction, Action<int> filterIndexAction)
        {
            fileNameAction(null);
