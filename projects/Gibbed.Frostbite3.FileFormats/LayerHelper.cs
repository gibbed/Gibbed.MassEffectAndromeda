...
namespace Gibbed.Frostbite3.FileFormats
{
    internal static class LayerHelper
    {
        ...
        public static void Read(Stream input, Action<Stream> callback)
        {
            ...
            if (Layers.ObfuscationLayer.IsObfuscated(data) == true)
            {
                var temp = Layers.ObfuscationLayer.Deobfuscate(data);
                if (data != input)
                {
                    data.Dispose();
                }
            ...
            else if (Layers.SignatureLayer.IsSigned(data) == true)
            {
                Layers.SignatureLayer.SkipSignature(data);
            }
